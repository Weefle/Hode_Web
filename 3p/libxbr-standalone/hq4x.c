...
static void
hq4x_32_rb(const uint32_t *sp, uint32_t srb, uint32_t *dp, uint32_t drb, int Xres, int Yres, const xbr_data *data) {
    int i, j, k;
...
void xbr_filter_hq4x(const xbr_params *ctx) {
    hq4x_32_rb((const uint32_t *) ctx->input, ctx->inPitch, (uint32_t *) ctx->output, ctx->outPitch, ctx->inWidth,
